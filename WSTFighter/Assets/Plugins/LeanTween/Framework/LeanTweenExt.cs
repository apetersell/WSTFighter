 ...
 //  Modified time: 2020-06-20
 //  Mail: blasineer@gmail.com
...
public static class LeanTweenExt
{
    ...
    public static LTDescr MoveBy(this GameObject go, Vector3 offset, float time = 0)
    {
        return LeanTween.move(go, go.transform.position + offset, time);
    }
    public static LTDescr MoveByX(this GameObject go, float offsetX, float time = 0)
    {
        return LeanTween.moveX(go, go.transform.position.x + offsetX, time);
    }
    public static LTDescr MoveByY(this GameObject go, float offsetY, float time = 0)
    {
        return LeanTween.moveY(go, go.transform.position.y + offsetY, time);
    }
    public static LTDescr MoveByZ(this GameObject go, float offsetZ, float time = 0)
    {
        return LeanTween.moveZ(go, go.transform.position.z + offsetZ, time);
    }
    public static LTDescr MoveLocalBy(this GameObject go, Vector3 offset, float time = 0)
    {
        return LeanTween.moveLocal(go, go.transform.localPosition + offset, time);
    }
    public static LTDescr MoveLocalByX(this GameObject go, float offsetX, float time = 0)
    {
        return LeanTween.moveLocalX(go, go.transform.localPosition.x + offsetX, time);
    }
    public static LTDescr MoveLocalByY(this GameObject go, float offsetY, float time = 0)
    {
        return LeanTween.moveLocalY(go, go.transform.localPosition.y + offsetY, time);
    }
    public static LTDescr MoveLocalByZ(this GameObject go, float offsetZ, float time = 0)
    {
        return LeanTween.moveLocalZ(go, go.transform.localPosition.z + offsetZ, time);
    }
    ...
    public static void Add(this MonoBehaviour go, LeanEvent eventEnum, System.Action<LTEvent> callback)
    {
        LeanTween.addListener(go.gameObject, (int)eventEnum, callback);
    }
    ...
    public static void dispatch(this MonoBehaviour go, LeanEvent eventEnum)
    {
        LeanTween.dispatchEvent((int)eventEnum);
    }

    public static void dispatch(this MonoBehaviour go, LeanEvent eventEnum, object data)
    {
        LeanTween.dispatchEvent((int)eventEnum, data);
    }

    public static LTDescr After(this MonoBehaviour go, float delayTime, System.Action callback) { return LeanTween.delayedCall(go.gameObject, delayTime, callback); }

    public static LTDescr After(this MonoBehaviour go, float delayTime, System.Action<object> callback) { return LeanTween.delayedCall(go.gameObject, delayTime, callback); }

    //LeanTween.delayedCall with repeat
    public static LTDescr Every(this MonoBehaviour go, float delayTime, System.Action callback)
    {
        return LeanTween.delayedCall(go.gameObject, delayTime, callback).setRepeat(-1);
    }

    public static LTDescr Tween(this MonoBehaviour go, float from, float to)
    {
        return LeanTween.value(go.gameObject, from, to, 0.5f);
    }

    public static LTDescr setTweenId(this LTDescr descr, ref TweenID tweenId)
    {
    ...
    public static LTDescr UpdateRatio(this LTDescr descr, float ratioPassed)
    {
        descr.passed = descr.time * ratioPassed;
        descr.updateNow();
        return descr;
    }

    public static LTDescr AddRatio(this LTDescr descr, float ratioPassed)
    {
        float newPass = descr.passed + descr.time * ratioPassed;
        if (newPass > 0 && newPass < descr.time)
        {
            descr.updateNow();
        }
        return descr;
    ...
    static public LTDescr UseStopMotion(this LTDescr descr)
    {
        LeanTweenManualTime.LazyInit();
        descr.setUseManualTime(true);
        return descr;
    }
}
...
{
    static public int NULL_TWEEN_ID = -1;

    public bool isTweening => LeanTween.isTweening(unwrapValue);
    public bool isNotTweening => !LeanTween.isTweening(unwrapValue);
    
    int? ID;
    int unwrapValue => ID ?? NULL_TWEEN_ID;

    public LTDescr tween
    {
        get
        {
            if (unwrapValue == NULL_TWEEN_ID)
                return null;
            else
                return LeanTween.descr(unwrapValue);
        }
    }

    static public implicit operator TweenID(int value)
    {
        return new TweenID { ID = value };
    }

    static public implicit operator int(TweenID input)
    {
        return input.unwrapValue;
    }

    public void Cancel()
    {
        LeanTween.cancel(unwrapValue);
    }

    public LTDescr UpdateRatio(float ratioPassed)
    {
        return tween?.UpdateRatio(ratioPassed);
    }

    public void SkipToEnd()
    {
        LeanTween.skipToEnd(unwrapValue);
    }
}

public partial class LeanTween : MonoBehaviour
{
    public static bool HasFinishedThisFrame()
    {
        return LeanTween.frameRendered == Time.frameCount;
    }

    public static LTDescr moveBy(GameObject go, Vector3 offset, float time)
    {
        return LeanTween.move(go, go.transform.position + offset, time);
    }

    public static LTDescr moveByX(GameObject go, float offsetX, float time)
    {
        return LeanTween.moveX(go, go.transform.position.x + offsetX, time);
    }

    public static LTDescr moveByY(GameObject go, float offsetY, float time)
    {
        return LeanTween.moveY(go, go.transform.position.y + offsetY, time);
    }

    public static LTDescr moveByZ(GameObject go, float offsetZ, float time)
    {
        return LeanTween.moveZ(go, go.transform.position.z + offsetZ, time);
    }

    public static LTDescr moveLocalBy(GameObject go, Vector3 offset, float time)
    {
        return LeanTween.moveLocal(go, go.transform.position + offset, time);
    }

    public static LTDescr moveLocalByX(GameObject go, float offsetX, float time)
    {
        return LeanTween.moveLocalX(go, go.transform.position.x + offsetX, time);
    }

    public static LTDescr moveLocalByY(GameObject go, float offsetY, float time)
    {
        return LeanTween.moveLocalY(go, go.transform.position.y + offsetY, time);
    }
    ...
    public static LTSeq sequence(ref TweenID tweenID)
    {
        tweenID.Cancel();
    ...
    public static LTDescr instantScale(GameObject go, float factor)
    {
        var ogrScale = go.transform.localScale;
        go.transform.localScale = ogrScale * factor;
        return LeanTween.scale(go, ogrScale, 0.2f);
    }

    public static void skipToEnd(int tweenId)
    {
        if (LeanTween.isTweening(tweenId))
        {
            var tween = LeanTween.descr(tweenId);
            if (tween.loopType == LeanTweenType.pingPong)
            {
                tween.UpdateRatio(0f);
                LeanTween.cancel(tweenId);
            }
            else
            {
                tween.UpdateRatio(1f);
                LeanTween.cancel(tweenId);
            }
        }
    }
